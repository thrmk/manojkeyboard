...
button =html.Div(
           dbc.Row(
              [
                  dbc.Col(dbc.Button("Login",color="primary",className="mb-3",href="#"),style={"padding-left":"10rem","width":"5rem"})]))
navbar = dbc.Navbar(
           html.Div(
...
df = pd.read_sql("select * from devices",cnx)

#print( "dev id=",df['devId'])
def graph():
  ...
  return dbc.Container([
                        ...
                        dcc.Graph(
                          ...
                          #  figure = go.Figure(go.Scatter(x = df['dateandtime'], y = df['rphvol'],)),
                            ...
                            figure={
                                'data': [
                                    {'x': [df['tstamp'],df['devId']=="1"], 'y': df['rphvol'], 'type': 'bar', 'name': 'rphvol'},
                                    {'x': [df['tstamp'],df['devId']=="1"], 'y': df['yphvol'], 'type': 'bar', 'name': 'yphvol'},
                                    {'x': [df['tstamp'],df['devId']=="1"], 'y': df['bphvol'], 'type': 'bar', 'name': 'bphvol'},
                                    {'x': [df['tstamp'],df['devId']=="1"], 'y': df['avgvol'], 'type': 'bar', 'name': 'avgvol'}#if df['devId']  =="1",

            ],
            'layout': {
                'title': 'Voltage Visualization'
                }}
                        )]#for dev=1 DeviceModel.query.all()
)
# else:
 #    return html.P("This device has no values for plotting graph")
...
app.layout = html.Div([navbar,content,data1,graph(),dcc.Location(id="url",refresh=False)])
